...
int Bmp::GetHeight() //높이를 가져와준다
{
	return (m_nHeight);
}

unsigned char* Bmp::GetBMPBuffer()
{
	return m_pBuffer;
}

int Bmp::GetWidth() //가로의 길이를 가져와준다
{
	return (m_nWidth);
}

bool Bmp::LoadBMPFile(char* filename) //비트맵의 정보를 불러와 버퍼에 색상정보를 담아주고 24bit이미지를 32bit이미지로 바꿔줌
{
	HANDLE hfile;
	DWORD actualRead;
...
bool Bmp::CopyBufferToSurface(LPDIRECTDRAWSURFACE7 lpSurface) //버퍼에서 표면으로 복사하는 함수
{
...
bool Bmp::CopyBufferToSurface(LPDIRECTDRAWSURFACE7 lpSurface, unsigned char* _buffer) //버퍼에서 표면으로 복사하는 함수
{
	if (!_buffer) //버퍼가 없으면
		return false; //false를 반환
...
bool Bmp::SaveWorldBMP(int _x, int _y, unsigned char* buffer, unsigned char* buffer2)
{
			...
			if (*(m_pBuffer + (x << 2) + nDestY2 * (m_nWidth << 2)) == 0)
				if (*(m_pBuffer + (x << 2) + nDestY2 * (m_nWidth << 2) + 1) == 0)
					if (*(m_pBuffer + (x << 2) + nDestY2 * (m_nWidth << 2) + 2) == 0)
						continue;
			CopyMemory(buffer2 + ((x + _x) << 2) + ((nDestY - _y)*(nWidth << 2)) - (x + _x + (nDestY - _y)*nWidth),
			           m_pBuffer + (x << 2) + nDestY2*(m_nWidth << 2), 3);
		}

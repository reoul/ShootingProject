...
bool CWorldMap::LoadBMPFile(char *filename)		//비트맵의 정보를 불러와 버퍼에 색상정보를 담아주고 24bit이미지를 32bit이미지로 바꿔줌
{
	HANDLE hfile;
	DWORD actualRead;
...
bool CWorldMap::CopyBufferToSurface(LPDIRECTDRAW7 pDirectDraw)		//버퍼에서 표면으로 복사하는 함수
{
...
bool CWorldMap::CopyBufferToSurface2(LPDIRECTDRAW7 pDirectDraw)		//버퍼에서 표면으로 복사하는 함수
{
...
bool CWorldMap::CopyBufferToSurface3(LPDIRECTDRAW7 pDirectDraw)		//버퍼에서 표면으로 복사하는 함수
{
